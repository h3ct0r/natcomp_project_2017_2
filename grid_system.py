...
class GridSystem(object):
    def __init__(self, cfg):
        ...
        print "[INFO]", "World size: {}x{}".format(self.col_number, self.row_number)
    ...
    def start(self):
        ...
        for repetition in xrange(self.cfg["repetitions"]):
                    ...
                    if self.draw_grid:
                        ...
                        for ant in self.colony_s.get_ants():
                            ...
                            if ant.state is "return_to_location":
                                pygame.draw.circle(self.screen, self.colors['yellow'], circle_pos, int(radius / float(2)) -1)
                            pass
            ...
            print "[INFO]", "Rep:{} iterations:{}".format(repetition, iterations)
            results.append(iterations)
            self.cfg['seed'] = self.cfg['seed'] + 1

        # when all repetitions are finished save the results to json file
        current_date = datetime.datetime.now().strftime("%I-%M%p_%d-%b-%Y")
        ...
        print "[INFO]", current_date
        print "[INFO]", self.cfg['config_file']
        print "[INFO]", "Full_results:", full_results
